...
process.source = cms.Source("PoolSource",
	fileNames = cms.untracked.vstring('file:DATA_PAT.root')
)
...
process.analysis = cms.EDAnalyzer("TopTreeProducer",
	myConfig = cms.PSet(
		...
		RootFileName = cms.untracked.string("DATA_TOPTREE.root"),
...
#		signalGenerator = cms.untracked.string('MADGRAPH'),
	...
	producersNames = cms.PSet(
		...
		vpfJetProducer = cms.untracked.vstring("selectedPatJetsPF2PAT","selectedPatJetsPF2PATNoLeptonCleaning"),
		vJPTJetProducer = cms.untracked.vstring(""),
		vmuonProducer = cms.untracked.vstring("selectedPatMuonsPF2PAT","selectedPatMuonsPF2PATNoLeptonCleaning"),
		velectronProducer = cms.untracked.vstring("selectedPatElectronsPF2PAT","selectedPatElectronsPF2PATNoLeptonCleaning"),# if electronTriggerMatching == true, change the electron inputTag to "cleanPatElectronsTriggerMatch"
		CalometProducer = cms.InputTag("patMETsAK5Calo"),
		vpfmetProducer = cms.untracked.vstring("patType1CorrectedPFMet","patMETsPF2PAT","patMETsPF2PATNoLeptonCleaning","patType1CorrectedPFMetNoLeptonCleaning"), #previously: PFmetProducer = cms.InputTag("patMETsPF2PAT"), which is not Type 1 corrected. Note that patType1CorrectedPFMet is still using PF2PAT.
		TCmetProducer = cms.InputTag("patMETsTC"),
		genEventProducer = cms.InputTag("genEvt"),
		generalTrackLabel = cms.InputTag("generalTracks"),
		electronNewId = cms.untracked.bool(False) #for recent electronID recommended by EGamma. still Not accepted by Top group
	)
)
